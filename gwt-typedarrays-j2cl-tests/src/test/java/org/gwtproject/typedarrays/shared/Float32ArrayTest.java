...
public class Float32ArrayTest {
  ...
  public void testBasic() {
    ...
    for (int i = 0; i < len; ++i) {
      assertEquals(0.0f, array.get(i), 0);
    }
  ...
  protected void validateArrayContents(Float32Array array, int offset) {
    for (int i = 0; i < offset; ++i) {
      assertEquals("index " + i, 0.0f, array.get(i), 0);
    }
    assertEquals("Index " + offset, 1.0f, array.get(offset++), 0);
    ...
    assertEquals("Index " + offset, Float.MAX_VALUE, array.get(offset++), 0);
