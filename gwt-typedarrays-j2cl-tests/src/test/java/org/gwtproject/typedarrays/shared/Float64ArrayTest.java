...
public class Float64ArrayTest {
  ...
  public void testBasic() {
    ...
    for (int i = 0; i < len; ++i) {
      assertEquals(0.0, array.get(i), 0);
    }
    ...
    view.setInt32(8, 0);
    assertEquals(-256.0, array.get(0), 0);
    assertEquals(1.0, array.get(1), 0);
  }
  ...
  protected void validateArrayContents(Float64Array array, int offset) {
    for (int i = 0; i < offset; ++i) {
      assertEquals("index " + i, 0.0, array.get(i), 0);
    }
    assertEquals("Index " + offset, 1.0, array.get(offset++), 0);
    ...
    assertEquals("Index " + offset, Double.MAX_VALUE, array.get(offset++), 0);
