...
/**
 * JS-specific utility methods, for use in client-side code that has the values in JS objects
 * already.
 *
 * <p>
 *
...
public class JsUtils {
  ...
  /**
   * Creates an {@link ArrayBuffer} from a string, with bytes encoded as individual characters
   * (which means in UTF8-encoded strings, byte values {@code 0x80-0xFF} take two bytes.
   *
   * @param str
   * @return an {@link ArrayBuffer} containing the bytes decoded from the string
  ...
  /**
   * Creates a string from an {@link ArrayBuffer}, with bytes encoded as individual characters
   * (which means in UTF8-encoded strings, byte values {@code 0x80-0xFF} take two bytes.
   *
   * @param buf
